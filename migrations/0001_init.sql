-- D1 schema for csp-report-worker.
-- Applied automatically at cold start by src/db.ts.
-- This file is the readable / `wrangler d1 execute` form;
-- the same statements are compiled into src/migrations.ts for the runtime.
...
-- Issue rows are deterministic: id = "<property_id>:<fingerprint>".
-- Same fingerprint on two properties is intentionally two distinct issues.
...
-- Per-issue rolling event sample. Capped at EVENT_SAMPLE_CAP (default 100)
-- per issue by a delete-oldest pass on each insert.
-- NO ip column — Cloudflare context only (country, ASN, colo, ray).
